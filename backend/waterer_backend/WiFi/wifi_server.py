...
def get_pump_manager(request: web.Request) -> WiFiPumpManager:

    manager = request.app[PUMP_MANAGER_KEY]
    assert isinstance(manager, WiFiPumpManager)
...
def create_app(manager: WiFiPumpManager) -> web.Application:
    ...
    async def default_route(request: web.Request):
        ...
        manager = request.app[PUMP_MANAGER_KEY]
        assert isinstance(get_pump_manager(request), WiFiPumpManager)
